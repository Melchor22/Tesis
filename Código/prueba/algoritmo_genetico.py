...
class AlgoritmoGenetico:
    ...
    def seleccionarPadres(self, numPadres):
        ...
        for i in range(numPadres):
            competidores = random.sample(disponibles, 2)
    ...
    def mutacion(self, hijos, PROBABILIDAD_MUTA, NUM_MUTACIONES):
        for individuo in hijos:
            random.seed()
            for i in range(NUM_MUTACIONES):
                if random.random() <= PROBABILIDAD_MUTA:
                    indice1, indice2 = random.sample(range(len(individuo.vectorRepresentacion)), 2)
                    individuo.vectorRepresentacion[indice1], individuo.vectorRepresentacion[indice2] = individuo.vectorRepresentacion[indice2], individuo.vectorRepresentacion[indice1]
